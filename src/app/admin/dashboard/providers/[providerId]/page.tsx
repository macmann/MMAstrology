    ...
    <main className="cosmic-page cosmic-scroll-page px-4 py-10 text-slate-50 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-4xl flex-col gap-8">
        <header className="rounded-[2rem] border border-white/15 bg-white/[0.08] p-6 shadow-2xl shadow-violet-950/30 backdrop-blur-xl sm:p-8">
          ...
          <p className="mt-6 text-xs font-semibold uppercase tracking-[0.35em] text-amber-200">
            Provider details
          </p>
          <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-black tracking-tight text-white sm:text-5xl">
                {providerConfig.displayName || providerConfig.name}
              </h1>
              <p className="mt-3 text-sm leading-6 text-violet-100/75">
                Update the user-facing provider name and description, enable or
                disable this provider, and edit the system prompt sent before
                every chat response.
              </p>
              <p className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                Internal provider key: {providerConfig.name}
              </p>
            </div>
            <span
        ...
        <form
          ...
          className="rounded-[2rem] border border-white/15 bg-white/[0.08] p-6 shadow-2xl shadow-violet-950/20 backdrop-blur-xl sm:p-8"
        >
          <div className="flex flex-col gap-5">
            <label className="flex items-start gap-3 rounded-2xl border border-white/15 bg-[#100a29]/80 p-4">
              ...
              <span>
                ...
                <span className="mt-1 block text-sm leading-6 text-violet-100/70">
                  Disabled providers are blocked from chat and return an
                  unavailable message to users.
                </span>
              </span>
            </label>
            ...
            <label className="block text-sm font-bold text-slate-300">
              ...
              <input
                ...
                placeholder="400"
              />
              <span className="mt-2 block text-xs leading-5 text-violet-100/60">
                Limits every chat response from this provider. Default is{" "}
            ...
            <div className="rounded-2xl border border-white/10 bg-[#100a29]/60 p-4 text-sm leading-6 text-violet-100/70">
              <p>
                Birth details are appended automatically at chat time, so this
                prompt should focus on persona, tone, behavior, and safety
                instructions.
              </p>
              <p className="mt-2 text-xs uppercase tracking-[0.2em] text-slate-500">
                Last updated{" "}
                {providerConfig.updatedAt.toISOString().slice(0, 10)}
              </p>
            </div>
